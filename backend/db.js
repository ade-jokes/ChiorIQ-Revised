...
if (process.env.DB_FILE) {
  ...
  DB_FILE = path.isAbsolute(configuredDbFile)
    ? configuredDbFile
    : path.resolve(__dirname, configuredDbFile);
} else if (IS_TEST_ENV) {
  // Test environment uses local directory
  ...
  DB_FILE = path.join(dataDir, 'choiriq.sqlite');
}
...
const SCHEMAS = {
  ...
  leaderAccessCodes: {
    table: 'leaderAccessCodes',
    columns: ['id', 'choirId', 'code', 'createdBy', 'createdAt', 'expiresAt', 'usedAt', 'consumedBy']
  }
};
...
function initSchema() {
  sqlite.exec(`
    ...
    CREATE TABLE IF NOT EXISTS leaderAccessCodes (
      id TEXT PRIMARY KEY,
      choirId TEXT NOT NULL,
...
function createLeaderAccessCode({ choirId, createdBy, ttlMinutes = 30 }) {
  if (!choirId || !createdBy) {
    throw new Error('choirId and createdBy are required to create a leader access code.');
  }

  let code = randomAccessCode();
  while (sqlite.prepare('SELECT 1 FROM leaderAccessCodes WHERE code = ?').get(code)) {
  ...
  const created = {
    ...
    createdBy,
    createdAt: now(),
    expiresAt: new Date(Date.now() + Number(ttlMinutes || 30) * 60 * 1000).toISOString(),
    usedAt: null,
    consumedBy: null
  };

  insertOrReplace('leaderAccessCodes', created);
  return findById('leaderAccessCodes', created.id);
}

function consumeLeaderAccessCode({ choirId, code, consumedBy }) {
  if (!choirId || !code || !consumedBy) {
    return null;
  }

  const normalized = String(code).trim().toUpperCase();
  const row = sqlite
    .prepare('SELECT * FROM leaderAccessCodes WHERE choirId = ? AND code = ?')
    .get(choirId, normalized);
  ...
  sqlite
    .prepare('UPDATE leaderAccessCodes SET usedAt = ?, consumedBy = ? WHERE id = ?')
    .run(now(), consumedBy, row.id);

  return findById('leaderAccessCodes', row.id);
}
...
function resetAll() {
  ...
  const tx = sqlite.transaction(() => {
    ...
    sqlite.prepare('DELETE FROM leaderAccessCodes').run();
  });
  tx();
}
...
module.exports = {
  ...
  createAnnouncement,
  createNote,
  createLeaderAccessCode,
  consumeLeaderAccessCode,
  seedDefaultSessions,
  resetAll,
  randomJoinCode,
  randomAccessCode,
